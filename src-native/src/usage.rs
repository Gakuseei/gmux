...
pub fn get_session_files() -> Vec<PathBuf> {
...
pub fn parse_session_file(path: &PathBuf, cutoff: DateTime<Utc>) -> Option<SessionUsage> {
...
mod tests {
    ...
    #[test]
    fn test_get_usage_data_today() {
        let result = get_usage_data("today".to_string());
        assert!(result.is_ok());
        let data = result.unwrap();
        let _ = data.total_input;
        let _ = data.total_output;
        let _ = data.total_cache_read;
        let _ = data.total_cache_write;
    }

    #[test]
    fn test_get_session_files_returns_vec() {
        let _files = get_session_files();
    }
}
