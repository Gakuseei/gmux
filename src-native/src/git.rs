...
mod tests {
    ...
    fn validate_repo_path_rejects_traversal() {
        let tmp = std::env::temp_dir().join("gmux_git_test_traversal");
        let _ = fs::create_dir_all(&tmp);
        let result = validate_repo_path(&tmp, "../../../etc/passwd");
        assert!(result.is_err());
        let _ = fs::remove_dir_all(&tmp);
    }

    #[test]
    fn validate_repo_path_accepts_valid() {
        let tmp = std::env::temp_dir().join("gmux_git_test_valid");
        let _ = fs::create_dir_all(tmp.join("subdir"));
        let result = validate_repo_path(&tmp, "subdir");
        assert!(result.is_ok());
        let _ = fs::remove_dir_all(&tmp);
    }

    #[test]
    fn get_current_branch_errors_for_non_repo() {
        let tmp = std::env::temp_dir().join("gmux_git_test_no_repo");
        let _ = fs::create_dir_all(&tmp);
        let result = get_current_branch(tmp.to_string_lossy().to_string());
        assert!(result.is_err());
        let _ = fs::remove_dir_all(&tmp);
    }
}
